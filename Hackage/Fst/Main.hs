...
oldmain n = do
  ...
  case args of
    ...
    as -> do
      ret <- runErrorT (batchMode (n == 1) as)
      case ret of
        Left err -> deepseq err $ when (n == 1) (putStrLn err)
        Right _  -> return ()
      oldmain (n-1)
...
batchMode :: Bool -> [String] -> ErrorT String IO ()
batchMode fibonShow cmdopt = do
  ...
  let action     = if isUpB cmd then upBFibon else downB
...
upBFibon :: Transducer String -> String -> String
upBFibon transducer str =
