...
%% Load a term using CPSA's parsing rules for terms.

load_term(Term, Internal) :-
	atom(Term),
	load_atom(Term, Internal).
load_term(Term, Term) :-
	integer(Term),
	Term >= 0.
...
%% Convert variables into uppercase atoms, and constants into
%% lowercase atoms.

load_atom('""', []) :- % The name for a listener is the null constant.
        !.
load_atom(Atom, Symbol) :-
